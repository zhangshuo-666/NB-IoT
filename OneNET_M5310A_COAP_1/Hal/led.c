...
void Led_Init(void)
{
	...
	gpioInitStrcut.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5 |GPIO_Pin_6 |GPIO_Pin_7;					//初始化Pin7、8、10
	gpioInitStrcut.GPIO_Speed = GPIO_Speed_50MHz;									//承载的最大频率
	GPIO_Init(GPIOA, &gpioInitStrcut);												//初始化GPIOC
...
void Led1_Set(LED_ENUM status)
{
	...
	GPIO_WriteBit(GPIOA, GPIO_Pin_4, status != LED_ON ?Bit_RESET:Bit_SET);	//status如果不等于LED_ON则返回Bit_SET，否则返回Bit_RESET。下同
